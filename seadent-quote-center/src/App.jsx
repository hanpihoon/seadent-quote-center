...
const [isLoggedIn, setIsLoggedIn] = React.useState(false);
  ...
  const exportQuotePdf = () => {
    ...
    const firstProductInfoRow = productInfoCards.slice(0, 2).join("");
    const remainingProductInfoRows = productInfoCards.slice(2).join("");

    const productInfoRows = productInfoCards.length
      ? `<div class="product-info-section"><div class="product-info-intro"><div class="product-info-header"><div class="product-info-title">THUYẾT MINH KỸ THUẬT SẢN PHẨM</div><div class="product-info-subtitle">Mỗi sản phẩm hiển thị hình ảnh và mã QR tài liệu kỹ thuật tương ứng</div></div><div class="product-info-first-row">${firstProductInfoRow}</div></div>${remainingProductInfoRows ? `<div class="product-info-grid">${remainingProductInfoRows}</div>` : ""}</div>`
      : "";
